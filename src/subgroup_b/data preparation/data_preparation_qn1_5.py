...
def encode_iot_data(df_iot):
    """
    Encodes categorical columns in the IoT dataset using Label Encoding.

    Args:
        df_iot (pd.DataFrame): IoT DataFrame to be encoded.

    Returns:
        pd.DataFrame: IoT DataFrame with encoded categorical columns.
    """
    # Define categorical columns
    categorical_cols = ['Visitor_ID', 'Loyalty_Member', 'Gender', 'Theme_Zone_Visited', 'Attraction', 'Day_of_Week']

    # Apply label encoding
    for col in categorical_cols:
        df_iot[col] = df_iot[col].astype(str)  # Ensure string dtype
        le = LabelEncoder()
...
df_iot_encoded.to_pickle(save_path)
...
df_all_combined_processed.to_pickle(survey_iot_weather_path)

print(f"Survey and weather processed data saved to {survey_weather_path}")
print(f"Survey, IoT, and weather processed data saved to {survey_iot_weather_path}")
