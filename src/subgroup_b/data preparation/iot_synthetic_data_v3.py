...
def generate_synthetic_data(num_samples=5000):
    ...
    for _ in range(num_samples):
        ...
        for theme_zone in zones_to_visit:
            ...
            # Adjust times based on whether it's peak or non-peak
            if 12 <= current_time < 14:  # Peak hours
                # For peak hours, we add extra time to stay
                stay_duration += np.random.uniform(0.2, 0.5)  # Add 12-30 mins extra during peak hours
            else:  # Non-peak hours (adjust check-in and queue times)
                stay_duration -= np.random.uniform(0.15, 0.25)  # Reduce 10-15 mins during non-peak hours
            ...
            for attraction in attractions_visited:
                # Adjust queue time based on peak/non-peak hours
                if 12 <= current_time < 14:  # Peak hours
                    queue_time = np.random.uniform(40, 90) if attraction in POPULAR_ATTRACTIONS else np.random.uniform(20, 40)
                else:  # Non-peak hours
                    queue_time = np.random.uniform(10, 30) if attraction in POPULAR_ATTRACTIONS else np.random.uniform(5, 20)
...
data_df.to_csv("../../data/synthetic_iot_data_v3.csv", index=False, float_format="%.2f")
